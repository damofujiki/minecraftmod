package hinasch.mods.unlsagamagic.entity;
...
//        }
    ...
    public void setStoneFall(World world,int x2,int y2,int z2,int sw){
		int blid = world.getBlockId(x2, y2, z2);
		
		if(blid==Block.stone.blockID | blid==Block.cobblestone.blockID|blid==Block.cobblestoneMossy.blockID){
			if(y2-2>0){
				if(world.isAirBlock(x2, y2-1, z2)|world.isAirBlock(x2, y2-2, z2)){

					if(blid==Block.stone.blockID){
					
						world.setBlock(x2, y2, z2, UnsagaBlocks.blockFallStone.blockID, 0, 4);
					}
					if(blid==Block.cobblestone.blockID | blid==Block.cobblestoneMossy.blockID){
						world.setBlock(x2, y2, z2, Block.gravel.blockID, 0, 4);
					}
				}
				
			}
		}
		if(blid==Block.netherrack.blockID){
			if(y2-2>0){
				if(world.isAirBlock(x2, y2-1, z2)|world.isAirBlock(x2, y2-2, z2)){

					if(blid==Block.netherrack.blockID){
					
						world.setBlock(x2, y2, z2, UnsagaBlocks.blockFallStone.blockID, 5, 4);
					}
