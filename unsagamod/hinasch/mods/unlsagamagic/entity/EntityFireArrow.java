...
public class EntityFireArrow extends EntityThrowable implements IProjectile
{
    ...
    public EntityFireArrow(World par1World, EntityLivingBase par2EntityLiving, EntityLivingBase par3EntityLiving, float par4, float par5)
    {
        super(par1World);
        this.renderDistanceWeight = 10.0D;
    ...
    public EntityFireArrow(World par1World, EntityLivingBase par2EntityLiving, float par3)
    {
        super(par1World);
        this.renderDistanceWeight = 10.0D;
        this.shootingEntity = par2EntityLiving;
...
//        }
    ...
    public void setFlare(boolean par1){
    	this.isFlare = par1;
    }
    ...
    public void onUpdate()
    {
        ...
        if (this.inGround)
        {
            ...
            if (var18 == this.inTile && var19 == this.inData)
            {
            	...
            	this.worldObj.spawnParticle("largeexplode", this.xTile, this.yTile, this.zTile, 1.0D, 0.0D, 0.0D);
            	if(isFlare){
            		this.doFlare(this.worldObj, this.shootingEntity, this.xTile, this.yTile, this.zTile, this.amp);
            	}
                    this.setDead();
                //}
            }
                ...
                if (var4.entityHit != null)
                {
                    ...
                    if (var4.entityHit.attackEntityFrom(var21, var23))
                    {
                        if (var4.entityHit instanceof EntityLiving)
                        {
                        	...
                        	if(isFlare){
                        		this.doFlare(this.worldObj, this.shootingEntity, var24.posX, var24.posY, var24.posZ, this.amp);
                        	}
    ...
    public void doFlare(World world,Entity entity,double x,double y,double z,int par1){
    	int par2 = par1+world.rand.nextInt(3);
    	
    	world.createExplosion(entity, x, y, z, 1.3F, true);
    	
    	for(int i=0;i<par2;i++){
    		double rnd = (double)(world.rand.nextInt(3))+world.rand.nextDouble();
    		double bx = x-1+rnd;
    		double by = y-1+rnd;
    		double bz = z-1+rnd;
    		
    		world.createExplosion(entity, bx, by, bz, 1.3F, true);
    		if(world.isAirBlock((int)bx, (int)by, (int)bz)){
    			if(!world.isAirBlock((int)bx, (int)by-1,(int) bz)|!world.isAirBlock((int)bx, (int)by-2, (int)bz)){
    				world.setBlock((int)bx, (int)by, (int)bz, Block.lavaMoving.blockID, 0, 4);
    			}
    		}

    		this.doBurning(world,(int) bx,(int) by, (int)bz);
    	}
    ...
    private void doBurning(World world,int x,int y,int z){
    	for(int i=0;i<3;i++){
    		for(int j=0;j<3;j++){
    			for(int k=0;k<3;k++){
    				if(!world.isAirBlock(x, y, z)){
    					int fx = x+world.rand.nextInt(3)-1;
    					int fy = x+world.rand.nextInt(3)-1;
    					int fz = x+world.rand.nextInt(3)-1;
    					if(world.isAirBlock(fx, fy, fz)){
    						if(world.rand.nextBoolean()){
    							world.setBlock(fx,fy, fz, Block.fire.blockID, 0, 4);
    						}
    					}
    				}
    			}
