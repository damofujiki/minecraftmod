...
public class CauseAddVelocity extends CauseDamageBoundingbox{
	...
	public void takeEntityLiving(EntityLivingBase living,DamageSource source){
		if(helper.getSkill()==AbilityRegistry.swing){
			if(living.onGround){
		...
		if(helper.getSkill()==AbilityRegistry.earthDragon){
			if(living.onGround){
				living.attackEntityFrom(source, helper.getAttackDamage());
				helper.addPotionChance(45, living, Potion.moveSlowdown.id, 90, 1);

				living.setVelocity(0.0D, 0.0D,0.0D);
			}
			double d0 = helper.ownerSkill.posX - living.posX;
			double d1;

			for (d1 = helper.ownerSkill.posZ - living.posZ; d0 * d0 + d1 * d1 < 1.0E-4D; d1 = (Math.random() - Math.random()) * 0.01D)
			{
