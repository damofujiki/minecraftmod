...
public class EntityBoulder extends EntityThrowable implements IProjectile
{
    ...
    public EntityBoulder(World par1World, EntityLivingBase par2EntityLivingBase, EntityLivingBase par3EntityLivingBase, float par4, float par5)
    {
        super(par1World);
        this.renderDistanceWeight = 10.0D;
    ...
    public EntityBoulder(World par1World, EntityLivingBase par2EntityLivingBase, float par3)
    {
        super(par1World);
        this.renderDistanceWeight = 10.0D;
        this.shootingEntity = par2EntityLivingBase;
...
//        }
    ...
    public EntityBoulder(World par1World, EntityLivingBase par2EntityLivingBase, float par3,double offsetX,double offsetY,double offsetZ)
    {
        super(par1World);
        this.renderDistanceWeight = 10.0D;
        this.shootingEntity = par2EntityLivingBase;
...
//        }
        ...
        this.setSize(0.5F, 0.5F);
        this.setLocationAndAngles(par2EntityLivingBase.posX+offsetX, par2EntityLivingBase.posY+offsetY + (double)par2EntityLivingBase.getEyeHeight(), par2EntityLivingBase.posZ+offsetZ, par2EntityLivingBase.rotationYaw, par2EntityLivingBase.rotationPitch);
        this.posX -= (double)(MathHelper.cos(this.rotationYaw / 180.0F * (float)Math.PI) * 0.16F);
        this.posY -= 0.10000000149011612D;
        this.posZ -= (double)(MathHelper.sin(this.rotationYaw / 180.0F * (float)Math.PI) * 0.16F);
        this.setPosition(this.posX, this.posY, this.posZ);
    ...
    public EntityBoulder(World par1World, EntityLivingBase par2EntityLivingBase, float par3,boolean par4){
    	this(par1World,par2EntityLivingBase,par3);
    	this.isStoneShower = par4;
    }
    ...
    public void onUpdate()
    {
        ...
        if (this.inGround)
        {
            ...
            if (var18 == this.inTile && var19 == this.inData)
            {
            	...
            	onEntityinGround(this.worldObj,this.xTile,this.yTile,this.zTile);
                    this.setDead();
                //}
            }
            else
        ...
        else
        {
            ...
            if (var4 != null)
            {
                if (var4.entityHit != null)
                {
                    ...
                    {
                        var21 = DamageSource.causeIndirectMagicDamage(this, this);
                    }
                    else
                    {
                        var21 = DamageSource.causeIndirectMagicDamage(this, this.shootingEntity);
                    }
                ...
                else
                {
                    ...
                    this.playSound("mob.irongolem.hit", 1.0F,1.2F / (this.rand.nextFloat() * 0.2F + 0.9F));
                    this.inGround = true;
                    this.arrowShake = 7;
                    this.setIsCritical(false);

                    if (!this.inTile.isAir(worldObj, xTile, yTile, zTile))
                    {
    ...
    private void onEntityinGround(World world,int xtile,int ytile,int ztile) {

    	byte sw = 0;
    	if(!this.isStoneShower)return;
    	for(int ix=0;ix<this.rangeStoneShower;ix++){
    		for(int iz=0;iz<this.rangeStoneShower;iz++){
    			for(int iy=0;iy<this.rangeStoneShower;iy++){
    				int x2 = xtile-(this.rangeStoneShower/2)+ix;
    				int y2 = ytile-(this.rangeStoneShower/2)+iy;
    				int z2 = ztile-(this.rangeStoneShower/2)+iz;
    				if(!world.isAirBlock(x2, y2, z2)){

    					this.setStoneFall(world, x2, y2, z2,(int)sw);
    					sw=(byte)(sw+1);
    				}
    ...
    public void setStoneFall(World world,int x2,int y2,int z2,int sw){
		Block blid = world.getBlock(x2, y2, z2);
		
		if(blid==Blocks.stone | blid==Blocks.cobblestone|blid==Blocks.mossy_cobblestone){
			if(y2-2>0){
				if(world.isAirBlock(x2, y2-1, z2)|world.isAirBlock(x2, y2-2, z2)){

					if(blid==Blocks.stone){
					
						world.setBlock(x2, y2, z2, UnsagaBlocks.blockFallStone, 0, 4);
					}
					if(blid==Blocks.cobblestone | blid==Blocks.mossy_cobblestone){
						world.setBlock(x2, y2, z2, Blocks.gravel, 0, 4);
					}
				}
				
			}
		}
		if(blid==Blocks.netherrack){
			if(y2-2>0){
				if(world.isAirBlock(x2, y2-1, z2)|world.isAirBlock(x2, y2-2, z2)){

					if(blid==Blocks.netherrack){
					
						world.setBlock(x2, y2, z2, UnsagaBlocks.blockFallStone, 5, 4);
					}
    ...
    public void setRangeStoneShower(int par1){
    	if(par1<4){
    		par1=4;
    	}
    	if(par1>30){
