...
public class EntityFlyingAxeNew extends EntityThrowable implements IProjectile{
	...
	protected void onImpact(MovingObjectPosition var1) {
		...
		if(var1!=null){
			...
			if(var1.typeOfHit==MovingObjectPosition.MovingObjectType.ENTITY && var1.entityHit!=null){
				DamageSource ds = new DamageSourceUnsaga(null, this.getThrower(), this.getLPDamage(),DamageHelper.Type.SWORDPUNCH,this);
				var1.entityHit.attackEntityFrom(ds, this.getDamage());
				if(var1.entityHit instanceof EntityLivingBase){
					if(this.getKnockbackModifier()>0){
						var1.entityHit.addVelocity((double)(-MathHelper.sin(this.rotationYaw * (float)Math.PI / 180.0F) * (float)this.getKnockbackModifier() * 0.5F), 0.1D, 
	...
	private void onUpdateSkyDrive() {
		...
		if(this.getSkyDriveStatus()==2){
			...
			if(this.getTarget()!=null){
				...
				Vec3 vecent = this.worldObj.getWorldVec3Pool().getVecFromPool(target.posX, target.posY, target.posZ);
				Vec3 sub = vecaxe.subtract(vecent);
				sub.normalize();
				double d1 = 0.45D;
				this.motionX = sub.xCoord * d1;
				this.motionZ = sub.zCoord * d1;
				this.motionY = sub.yCoord * d1;
	...
	public void readEntityFromNBT(NBTTagCompound par1NBTTagCompound)
	{
		...
		this.setLPDamage(par1NBTTagCompound.getFloat("damage.LP"));
		this.tickAxe = par1NBTTagCompound.getInteger("tickAxe");
		this.setDrive(par1NBTTagCompound.getFloat("drive"));
		//this.knockbackModifier = par1NBTTagCompound.getInteger("knockbackModifier");
		this.updateSkyDriveStatus(par1NBTTagCompound.getInteger("statusSkyDrive"));
