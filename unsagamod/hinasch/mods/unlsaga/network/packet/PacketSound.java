	...
	public void handleClientSide(EntityPlayer player) {
		...
		if(id==PLAY_SOUND_AT_ENTITY){
			if(player instanceof EntityClientPlayerMP){
				...
				if(entity!=null){
					ep.worldObj.playSoundAtEntity(entity, StaticWords.soundMap.get(soundnumber), 1.0F, 1.0F / (ep.worldObj.rand.nextFloat() * 0.4F + 1.2F) + 1.0F * 0.5F);
		...
		if(id==PLAY_SOUND_AT_POS){
			if(player instanceof EntityClientPlayerMP){
				EntityClientPlayerMP ep = (EntityClientPlayerMP)player;
				ep.worldObj.playSound((double)pos.x, (double)pos.y, (double)pos.z, StaticWords.soundMap.get(soundnumber), 1.0F, 1.0F / (ep.worldObj.rand.nextFloat() * 0.4F + 1.2F) + 1.0F * 0.5F,false);
